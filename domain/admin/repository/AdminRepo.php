...
namespace domain\admin\repository;

use domain\admin\error\AdminRootError;
use domain\admin\model\AdminModel;
...
//{@block_br}
    ...
    public static function queryMatchIndex(QueryMatch $QM)
    {
        ...
        $QM->include($includeArr, AdminModel::class, $except);
        if (!empty($includeArr)) {
...
//{@block_r/}
    ...
    {
        $where = ['id' => $id];
        $field = [ 'id','role','user_id','name','avatar','sex','mobile','client_driver','lat','lng','status',
            'on_line_time','off_line_time' ];
