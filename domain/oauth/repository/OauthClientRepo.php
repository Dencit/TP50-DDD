...
use domain\oauth\model\OauthClientModel;
 ...
 * 调用原则: 向下调用[模型类]
 */
class OauthClientRepo extends BaseRepository
{
    //初始化 self::$query 模型对象
...
//{@block_br}
    ...
    public static function queryMatchIndex(QueryMatch $QM)
    {
        ...
        $filterArr = (new OauthClientModel())->getFieldKeys();
        ...
        $QM->include($includeArr, OauthClientModel::class, $except);
        if (!empty($includeArr)) {
...
//{@block_r/}
    ...
    public static function isOauthClientExist($scopeId,$clientId){
        $where=["scope_id"=>$scopeId,"client_id"=>$clientId];
