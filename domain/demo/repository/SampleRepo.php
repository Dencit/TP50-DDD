 ...
 * 调用原则: 向下调用[模型类]
 */
class SampleRepo extends BaseRepository
{
    //初始化 self::$query 模型对象
...
//{@block_br}
    ...
    public static function queryMatchIndex(QueryMatch $QM)
    {
        ...
        $filterArr = (new SampleModel())->getFieldKeys();
        ...
        $QM->include($includeArr, SampleModel::class, $except);
        if (!empty($includeArr)) {
...
//{@block_r}
    ...
    public function queryMatchRead(QueryMatch $QM)
    {
        ...
        //?_group=city_id,user_id
        $QM->group($groupArr);
        if (!empty($groupArr)) {
            $query->group(implode(",", $groupArr));
        }
