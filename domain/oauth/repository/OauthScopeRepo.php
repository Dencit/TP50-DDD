...
use domain\oauth\model\OauthScopeModel;
 ...
 * 调用原则: 向下调用[模型类]
 */
class OauthScopeRepo extends BaseRepository
{
    //初始化 self::$query 模型对象
    protected $model = OauthScopeModel::class;
...
//{@block_br}
    ...
    public static function queryMatchIndex(QueryMatch $QM)
    {
        ...
        $filterArr = (new OauthScopeModel())->getFieldKeys();
        ...
        $QM->include($includeArr, OauthScopeModel::class, $except);
        if (!empty($includeArr)) {
