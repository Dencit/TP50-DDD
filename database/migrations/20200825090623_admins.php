...
class Admins extends Migrator
{
    ...
    public function up(){
        ...
        // create the table
        $table = $this->table('admins',[ 'engine'=>'InnoDb','charset'=> 'utf8mb4', 'collation'=> 'utf8mb4_general_ci','comment'=>'管理员表']);

        $table
            ->addColumn('user_id','integer',['signed'=>false,'limit'=>MysqlAdapter::INT_BIG,'default'=>0,'comment'=>'关联用户id'])
            ->addColumn('role','string',['limit'=>255,'default'=>'admin','comment'=>'管理员角色: user,admin'])
            ->addColumn('name','string',['limit'=>255,'default'=>'','comment'=>'管理员名称'])
            ...
            ->addColumn('client_driver','text',['limit'=>MysqlAdapter::TEXT_REGULAR,'comment'=>'客户端信息:浏览器信息'])
