 ...
 * 调用原则: 向下调用[模型类]
 */
class OauthTokenRepo extends BaseRepository
{
    //初始化 self::$query 模型对象
    protected $model = OauthTokenModel::class;
...
//{@block_br}
    ...
    public static function queryMatchIndex(QueryMatch $QM)
    {
        ...
        $filterArr = (new OauthTokenModel())->getFieldKeys();
        ...
        $QM->include($includeArr, OauthTokenModel::class, $except);
        if (!empty($includeArr)) {
...
//{@block_r/}
    ...
    //是否存在没过期的token
    public function isClientTokenHave($userMark, $scopeId, $clientId)
    {
        $where  = ["user_mark" => $userMark, "scope_id" => $scopeId, "client_id" => $clientId];
        $field  = ['id', 'user_mark', 'token', 'client_secret', 'start_time', 'expire_time'];
        $field  = array_merge($field, array_keys($where));
        $result = self::$query->field($field)
            ->where($where)->where('expire_time', '>', date('Y-m-d H:i:s'))->find();
        return $result;
    }
